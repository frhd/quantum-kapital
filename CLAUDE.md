...
- **Core Modules**:
  ...
  - `services/`: Business logic layer
    ...
    - `decay_watcher/` (Phase 14; replaced Phase 18): `DecayWatcher` async trait with `check(&Setup) -> DecayDecision`. Phase 18 widened `DecayDecision` with `outcome: DecayOutcome { StillValid, Invalidated, TargetHit, ThesisChanged, Skipped }` alongside the existing `still_valid: bool, reason: Option<String>, suggested_action: Option<String>`. Constructor sugar: `DecayDecision::still_valid()` / `invalidate(reason)` / `target_hit(reason)` / `thesis_changed(reason)` / `skipped()`. Two impls ship: `DecayWatcherStub` (always `StillValid` — kept for tests and as a no-op fallback) and Phase 18's `LlmDecayWatcher` backed by Claude Haiku 4.5 via `LlmService`. Constructor `LlmDecayWatcher::new(llm: Arc<LlmService>, bars: Arc<dyn BarsFetcher>)` plus `with_clock(Arc<dyn DecayClock>)` for tests. Public API: `build_request(&Setup, &DecayContext<'_>) -> LlmRequest` (forced tool-use `emit_decay`, two ephemeral-cached system blocks — persona prompt + per-setup thesis context that embeds `setup_id` so the cache is keyed per-setup; user message carries the last 12 Min15 bars + the most recent close as `current_quote`), `parse_decision(&Value) -> Result<DecayDecision, DecayError>`, and the trait method `check(&Setup)` which (1) short-circuits to `Skipped` when `now - setup.detected_at < FRESHNESS_GRACE` (30 min — first few bars after detection are too noisy to evaluate), (2) fetches fresh `Min15` intraday bars (`Lookback::Days(1)`) and trims to the last 12, (3) calls `LlmService::message`, (4) parses the `emit_decay` tool input. Every transient/config failure (`BudgetExhausted`, `Auth`, `Upstream`, `Network`, `NoApiKey`, `Malformed`, `UnknownModel`, bars-fetch error, missing tool call, malformed input) collapses to `DecayDecision::skipped()` with a `warn!` so the scheduler never escalates an LLM hiccup into a spurious invalidation. Output schema (forced tool-use `emit_decay`): `{ still_valid, outcome (still_valid|invalidated|target_hit|thesis_changed), reason, suggested_action? }` — `still_valid` + `outcome` + `reason` required; `suggested_action` is informational only (the system never places orders). Source of truth for the system prompt + tool schema is `decay_watcher/mod.rs`; prompt versioning is logged in `impl/scratch/llm-prompts.md`. Wired in `lib.rs::run` as `Arc<dyn DecayWatcher> = Arc::new(LlmDecayWatcher::new(llm_service, decay_bars))` (a second `Arc<dyn BarsFetcher>` clone of `hist_service` is taken alongside the `TrackerRunner` clone). 8 unit tests in `tests.rs` cover request shape, forced tool-use, the four outcome parses, the freshness-grace skip, the budget kill-switch (returns `Skipped` without HTTP), and the per-setup ephemeral cache marker.
