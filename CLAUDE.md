...
- **Core Modules**:
  ...
  - `strategies/`: Strategy detector framework (added Phase 06). Pure types + trait + registry; concrete detectors land in Phases 07–09 (Phase 07 added the breakout detector).
    ...
    - `default_registry()` (in `strategies/mod.rs`, added Phase 07): seeds a registry with all production detectors (currently just `BreakoutDetector`); registration order is the canonical evaluation order.
    ...
    - `tests.rs`: 5 unit tests cover registry ordering, tag filtering, error collection, and target math; `breakout/tests.rs` adds 10 table-driven detector tests; `indicators.rs` carries 11 inline tests including the Wilder-1978 RSI reference fixture.
