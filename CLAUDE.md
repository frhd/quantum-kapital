...
- **Core Modules**:
  ...
  - `services/`: Business logic layer
    ...
    - `tracker_runner/` (Phase 10; extended Phase 15): `TrackerRunner` glues bars + news + the detector registry together for a single command-callable surface. Constructor `TrackerRunner::new(db, tracker, state_machine: Arc<TrackerStateMachine>, emitter: Arc<EventEmitter>, bars: Arc<dyn BarsFetcher>, news: Arc<dyn NewsFetcher>, registry: Arc<DetectorRegistry>)` (Phase 15 widened the signature to take `emitter`). `BarsFetcher` is blanket-impl'd for `HistoricalDataService`; `NewsFetcher` is blanket-impl'd for `FinancialDataService` (the news arm collapses errors to an empty `Vec` with a `warn!` so a missing API key never propagates). Public API: `context_for(symbol) -> OwnedMarketContext` (mandatory daily 200-day fetch; intraday Min15 for today and 24h news are best-effort; fundamentals + live quote intentionally `None` since no current detector reads them); `run_for(symbol) -> Vec<Setup>` (dispatches `registry.evaluate_all` against the borrowed `MarketContext` from `OwnedMarketContext::as_borrowed`, persists hits with 24h dedup keyed on `(symbol, strategy, direction)`, calls `state_machine.on_setup_detected` after each successful persist (Phase 12 — flips the ticker into `SetupActive` and extends `in_play_until`), emits `AppEvent::SetupDetected { setup, thesis: None }` (Phase 15; `thesis` stays `None` until Phase 17 wires the LLM thesis prompt — dedup hits do NOT re-emit), touches `last_checked_at` on success, logs detector / persistence / state-machine / emit errors as `warn!`); `run_all() -> Vec<RunResult>` iterates the watchlist excluding `CoolDown` rows and surfaces per-symbol failures inside individual `RunResult { symbol, setups, error }` entries instead of short-circuiting. `DUPLICATE_WINDOW = 24h`, `DAILY_LOOKBACK_DAYS = 200`, `INTRADAY_BAR_SIZE = Min15`. Wired in `lib.rs::run` as `Arc<TrackerRunner>` alongside a state-managed `Arc<FinancialDataService>` (constructed once with the env-supplied `ALPHA_VANTAGE_API_KEY` + shared `Arc<Db>`).
  ...
  - `config/`: Application configuration
    - `settings.rs`: Configuration management. Phase 14 added `AppConfig.tracker: TrackerConfig` (`#[serde(default)]`) with `intraday_tick_interval_secs: u64` (default 300) — consumed by `IntradayScheduler` at construction time in `lib.rs::run`.
...
- `tracker/` (Phase 05; extended Phase 15): Watchlist UI over the `tracker_*` Tauri commands. `TrackerTab` composes `Watchlist` (table with inline tag editing + remove + Phase 15 `SetupBadge`) and a status filter; mounted as a top-level tab with an unread-count badge in `App.tsx`. `AddToTrackerDialog` is a portal-rendered modal triggered by the manual "Add" button or the scanner row's "Add to tracker"; on duplicate it surfaces an "already tracked" message. `useWatchlist(refreshKey)` owns fetch/CRUD against `ibkrApi.tracker.*` and re-fetches whenever `refreshKey` changes (App.tsx bumps it after every dialog submission). Phase 15 added `useTrackerEvents()` which subscribes to `setup-detected` / `setup-invalidated` / `ticker-status-changed` via the Tauri `listen` API, keeps a capped 100-event log, exposes `lastSetupDetected` / `lastInvalidated` / `lastStatusChanged`, and maintains `activeSetupBySymbol` (latest active `Setup` per symbol; cleared on invalidate). `TrackerTab` consumes the hook to push `Toast` notifications via `useToasts()` (a lightweight in-memory toast manager rendered through a `ToastViewport` portal in `src/shared/components/ui/toast.tsx`) and to refresh the watchlist on each event so the row badge + status update without manual reload. Source-of-truth types live in `src/features/tracker/types.ts` mirroring the Rust enums in snake_case (`watching` / `in_play` / `setup_active` / `cool_down`; `manual` / `scanner` / `news`); Phase 15 added `Setup`, `Direction`, `TargetLevel`, `SetupStatus`, `SetupDetectedPayload`, `SetupInvalidatedPayload`, `TickerStatusChangedPayload`, and the union `TrackerEvent`.
