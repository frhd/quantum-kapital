...
- **Core Modules**:
  ...
  - `storage/`: SQLite layer for the Tracker subsystem (added Phase 01)
    ...
    - DB lives at `app_local_data_dir()/tracker.sqlite`; `Arc<Db>` is both `app.manage`d in `lib.rs::run` and held on `IbkrState` (Phase 04 wired `IbkrState::db` + `IbkrState::tracker: Arc<TrackerService>`; Phase 12 added `IbkrState::state_machine: Arc<TrackerStateMachine>`, constructed in `IbkrState::new`)
