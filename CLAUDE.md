...
- **Core Modules**:
  ...
  - `strategies/`: Strategy detector framework (added Phase 06). Pure types + trait + registry; concrete detectors land in Phases 07–09 (Phase 07 added the breakout detector).
    ...
    - `default_registry()` (in `strategies/mod.rs`, added Phase 07; expanded Phase 08): seeds a registry with all production detectors (currently `BreakoutDetector`, `EpisodicPivotDetector`); registration order is the canonical evaluation order.
    ...
    - `tests.rs`: 5 unit tests cover registry ordering, tag filtering, error collection, and target math; `breakout/tests.rs` adds 10 table-driven detector tests; `episodic_pivot/tests.rs` adds 12 table-driven detector tests; `indicators.rs` carries 11 inline tests including the Wilder-1978 RSI reference fixture.
