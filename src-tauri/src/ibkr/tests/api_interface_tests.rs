...
async fn test_market_data_snapshot() {
    ...
    assert!(spread > 0.0 && spread < 1.0, "Spread should be positive and reasonable");
}
...
async fn test_portfolio_analysis() {
    ...
    for position in &positions {
        let market_data = client.get_market_data_snapshot(&position.symbol).await.unwrap();
    ...
    let net_liq = account_values.iter()
...
async fn test_error_handling_for_new_interfaces() {
    ...
    assert!(client.get_historical_data(test_fixtures::sample_historical_data_request()).await.is_err());
    ...
    assert!(client.scan_market(test_fixtures::sample_scanner_subscription()).await.is_err());
}
...
async fn test_concurrent_api_calls() {
    ...
    let historical_future = client.get_historical_data(test_fixtures::sample_historical_data_request());
