...
async fn test_portfolio_value_calculation() {
    ...
    // Calculate totals
    let total_market_value: f64 = fetched_positions.iter().map(|p| p.market_value).sum();
    let total_unrealized_pnl: f64 = fetched_positions.iter().map(|p| p.unrealized_pnl).sum();
    let total_realized_pnl: f64 = fetched_positions.iter().map(|p| p.realized_pnl).sum();
