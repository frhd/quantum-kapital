...
use super::types::{IntendedPriceSource, IntentSide, IntentStatus, LinkageDecision, OrderIntent, SlippageRecord};
...
mod tests {
    ...
    fn long_paying_more_than_intended_is_positive_slippage() {
        let s = compute_slippage(101.50, 100_00, IntentSide::Buy);
    ...
    fn short_receiving_less_than_intended_is_positive_slippage() {
        ...
        let s = compute_slippage(98.50, 100_00, IntentSide::Sell);
        assert_eq!(s.bps, 150);
        assert_eq!(s.signed_cents_per_share, 150);
    }
    ...
    fn long_paying_less_than_intended_is_negative_signed_slippage() {
        ...
        let s = compute_slippage(99.50, 100_00, IntentSide::Buy);
        assert_eq!(s.bps, 50);
    ...
    fn slippage_zero_at_perfect_fill() {
        let s = compute_slippage(100.00, 100_00, IntentSide::Buy);
        assert_eq!(s.bps, 0);
    ...
    fn match_clean_fill_picks_only_open_intent() {
        let i = intent(IntentSide::Buy, 100.0, 100_00, 1);
        let f = fill("AAPL", ExecutionSide::Bought, 100.0, 100.50);
        let d = match_fill(&f, &[i.clone()]).unwrap();
    ...
    fn match_fifo_picks_earliest_open_intent() {
        let older = intent(IntentSide::Buy, 100.0, 100_00, 30);
        let newer = intent(IntentSide::Buy, 100.0, 100_00, 1);
    ...
    fn match_skips_expired_window() {
        let mut i = intent(IntentSide::Buy, 100.0, 100_00, 90);
    ...
    fn match_skips_wrong_side() {
        let i = intent(IntentSide::Sell, 100.0, 100_00, 1);
        let f = fill("AAPL", ExecutionSide::Bought, 100.0, 100.0);
        assert!(match_fill(&f, &[i]).is_none());
    }
    ...
    fn match_skips_filled_intent() {
        let mut i = intent(IntentSide::Buy, 100.0, 100_00, 1);
        i.matched_qty = 100.0;
        let f = fill("AAPL", ExecutionSide::Bought, 100.0, 100.0);
        assert!(match_fill(&f, &[i]).is_none());
    }
