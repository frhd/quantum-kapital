...
- [x] Phase 01 — `Db` and `tracked_tickers` table.
...
- [x] `add_inserts_row_and_returns_typed_value` — `add(symbol, source, source_meta, tags, notes)` returns a `TrackedTicker` whose fields match the inserted row.
- [x] `add_duplicate_symbol_errors` — second `add('AAPL', ...)` returns `TrackerError::AlreadyTracked`.
- [x] `remove_deletes_row` — `remove('AAPL')` then `list(None)` excludes it.
- [x] `remove_non_existent_is_idempotent` — `remove('NOSUCH')` returns `Ok(())` not an error.
- [x] `list_filters_by_status` — three rows in three statuses; `list(Some(InPlay))` returns one.
- [x] `set_tags_replaces_tag_array` — initial tags `[Breakout]`; `set_tags('AAPL', [EpisodicPivot, ParabolicShort])` overwrites; `list` reflects.
- [x] `set_status_updates_status_and_in_play_until` — set to `InPlay` with `in_play_until = now+3d`; row reflects.
- [x] `tags_round_trip_via_json` — custom variant `StrategyTag::Custom("squeeze")` serializes and deserializes.
- [x] `source_meta_round_trip` — JSON value with nested object survives a round trip.

## Implementation tasks

- [x] Create `src-tauri/src/ibkr/types/tracker.rs`:
  ```rust
...
- [x] Re-export `TrackedTicker`, `TrackerSource`, `TrackerStatus`, `StrategyTag` from `ibkr/types/mod.rs`.
- [x] Create `src-tauri/src/services/tracker_service.rs`:
...
- [x] Add `TrackerError` (`AlreadyTracked`, `NotFound`, `Storage(StorageError)`).
- [x] Wire into `IbkrState`:
...
- [x] Add Tauri commands in `commands/tracker.rs` (extend the file from Phase 02):
  - `tracker_add`, `tracker_remove`, `tracker_list`, `tracker_get`, `tracker_set_tags`, `tracker_set_status`.
- [x] Register all six commands in `lib.rs::run` `generate_handler!`.

## Verification

- [x] `cargo test --manifest-path src-tauri/Cargo.toml services::tracker_service` — green.
- [x] Manual via Tauri devtools console:
  ```
  ...
  ```
- [x] `sqlite3 ~/.local/share/quantum-kapital/tracker.sqlite "SELECT * FROM tracked_tickers"` shows expected state.
