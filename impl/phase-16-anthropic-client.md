...
- [ ] `sends_correct_headers` — request includes `x-api-key`, `anthropic-version: 2023-06-01`, `content-type: application/json`.
- [ ] `serializes_messages_correctly` — request body has `model`, `max_tokens`, `messages: [{role, content}]`, optional `system`, optional `tools`, optional `tool_choice`.
- [ ] `parses_text_response` — mock returns `{content: [{type: "text", text: "..."}], usage: {...}}`; `LlmService::message(...)` returns `LlmResponse { text, tool_calls, usage }`.
- [ ] `parses_tool_use_response` — mock returns `{content: [{type: "tool_use", name: "emit_thesis", input: {...}}]}`; `LlmService` returns the parsed input as `serde_json::Value`.
- [ ] `forced_tool_use_returns_typed_args` — passing `tool_choice = ForceTool("emit_thesis")` results in `tool_calls.first()` matching schema.
- [ ] `records_call_in_db_with_cost` — successful call inserts a row in `llm_calls` with `input_tokens`, `output_tokens`, `cache_read_tokens`, computed `cost_usd` based on model price table.
- [ ] `cost_calculator_handles_each_supported_model` — sonnet-4-6, haiku-4-5 → correct $/M token math.
- [ ] `prompt_cache_block_serializes_with_cache_control` — system prompt block with `cache: true` produces `{type: "text", text: "...", cache_control: {type: "ephemeral"}}`.
- [ ] `daily_budget_kill_switch_blocks_new_calls` — sum of today's `cost_usd` ≥ `daily_llm_budget_usd` → `LlmService::message` returns `Err(LlmError::BudgetExhausted)` without making an HTTP request.
- [ ] `kill_switch_resets_at_midnight_utc` — fixture clock past midnight; budget check uses today's date.
- [ ] `propagates_4xx_errors` — mock returns 401 → `LlmError::Auth`.
- [ ] `propagates_5xx_with_retry_disabled` — 500 → `LlmError::Upstream` after one attempt (no retries in this phase; can add backoff in a later iteration).

## Implementation tasks

- [ ] Add to `AppConfig::api`:
  ```rust
...
- [ ] Create `src-tauri/src/services/llm_service/mod.rs`:
  ```rust
...
- [ ] `LlmService` writes a row to `llm_calls` after every successful response.
- [ ] Wire into `IbkrState` (`pub llm: Arc<LlmService>`).

## Verification

- [ ] `cargo test --manifest-path src-tauri/Cargo.toml services::llm_service` — green.
